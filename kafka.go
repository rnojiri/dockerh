...
func CreateKafka(podName string, kafkaPort int, zookeeperHost string, zookeeperPort int) (string, error) {
	...
	return CreateCustomKafka(podName, "", "", kafkaPort, zookeeperHost, zookeeperPort, defaultWaitingTimeout)
}
...
func CreateKafkaInNetwork(podName, network string, kafkaPort int, zookeeperHost string, zookeeperPort int) (string, error) {
	...
	return CreateCustomKafka(podName, "", network, kafkaPort, zookeeperHost, zookeeperPort, defaultWaitingTimeout)
}

// CreateCustomKafka - starts the kafka pod using custom configurations
func CreateCustomKafka(podName, networkInspectFormat, network string, kafkaPort int, zookeeperHost string, zookeeperPort int, timeout time.Duration) (string, error) {
	...
	return WaitUntilListeningAndGetPodIP(podName, networkInspectFormat, network, kafkaPort, timeout)
}
